export const MAPLESTORY_USERKEY_DEFAULT = new Uint8Array([ // 16 * 8
...
export function isDefaultMapleStoryUserKey (): boolean {
  if (!(userKeyWzLib instanceof Uint8Array)) return false
  if (MAPLESTORY_USERKEY_DEFAULT.length !== userKeyWzLib.length) return false
  for (let i = 0; i < MAPLESTORY_USERKEY_DEFAULT.length; i++) {
    if (userKeyWzLib[i] !== MAPLESTORY_USERKEY_DEFAULT[i]) return false
  }
  return true
}

export const bShuffle = new Uint8Array([ // 16 * 16
...
export function getTrimmedUserKey (userKey: Uint8Array): Uint8Array {
