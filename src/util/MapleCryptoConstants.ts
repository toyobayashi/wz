export class MapleCryptoConstants {
  public static userKey: Buffer = Buffer.from([ // 16 * 8
    
  ])

  public static bShuffle: Buffer = Buffer.from([ // 16 * 16
    
  ])

  public static bDefaultAESKeyValue: Buffer = Buffer.from([
    0xC6, 0x50, 0x53, 0xF2, 0xA8, 0x42, 0x9D, 0x7F, 0x77, 0x09, 0x1D, 0x26, 0x42, 0x53, 0x88, 0x7C
  ])

  public static WZ_GMSIV: Buffer = Buffer.from([0x4D, 0x23, 0xC7, 0x2B])

  public static WZ_MSEAIV: Buffer = Buffer.from([0xB9, 0x7D, 0x63, 0xE9])

  public static WZ_OffsetConstant: number = 0x581C3F6D

  public static getTrimmedUserKey (): Buffer {
    const key = Buffer.alloc(32)
    for (let i = 0; i < 128; i += 16) {
      key[i / 4] = MapleCryptoConstants.userKey[i]
    }
    return key
  }
}
